...
-- estimate history along with the logs — the same failure 0015 fixed for
-- measured entries. `EstimatedEntry` is written by the Fio poll
-- (src/lib/fio.ts -> src/lib/payment-entry-estimate.ts) and is never cleaned up.
--
-- Deliberately no backfill statement here, unlike 0015: the estimate is an
-- unbounded-knapsack over the current price list, which SQL can't express.
